                ...
                if (Pattern.matches(regex, input)) {
                    ...
                    if (!(input == "total" || input == "done" || input == "redo" || input == "main" || input == "excel")) {
                        ...
                        getExcelInput@ while (true) {
                            ...
                            println("Would you like to resume adding times using regular mode, starting with Time$inputNumber, or continue using excel mode? Enter \"resume\" to resume adding with regular mode, or \"excel\" to continue entering file paths in the aforementioned format: ")
            ...
            mainInputs@ while (true) {
                if (Pattern.matches(hhCmmCss, input)) { //such as
                    ...
                    if (!(input == "total" || input == "done" || input == "redo" || input == "main" || input == "excel")) {
                        ...
                        println("Enter the file path to a .txt file which contains all of the times you would like to add with each time on a new line. To see an example file, type \"example\".")
                        var file: File
                        getExcelInput@ while (true) {
                            getFilePath@ while (true) {
                                val filename =
                            ...
                            val times = file.readLines()
                            val listofHourMinuteAndSecondTotalfromfile =
                                parseExcelTimesMulti(times) ?: continue@getExcelInput
                            hour += listofHourMinuteAndSecondTotalfromfile[0]
                            minute += listofHourMinuteAndSecondTotalfromfile[1]
                            second += listofHourMinuteAndSecondTotalfromfile[2]
                            println("Would you like to resume adding times using regular mode, starting with Time$inputNumber, or continue using excel mode? Enter \"resume\" to resume adding with regular mode, or \"excel\" to continue entering file paths in the aforementioned format: ")
                    ...
                    timeB@while (true) {
                        if (Pattern.matches(hhCmmCssAMPM, timeB)) {
                            ...
                            if (!(timeB == "total" || timeB == "done" || timeB == "redo" || timeB == "main" || timeB == "excel")) {
                                ...
                                println("Proceeding with excel mode now will delete your previous entry for Time${i}a. Would you still like to continue? Enter \"yes\" to continue with excel mode and \"no\" to resume adding in regular mode with Time$i$ab")
                                wantToProceed@ while (true) {
                                    val answer = getInput()
                                    if (answer == "yes") {
                                        timeA = ""
                                        timeB = ""
